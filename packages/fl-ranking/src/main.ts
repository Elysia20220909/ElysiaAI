...
function showPlayerProfile(player: PlayerStats) {
  const leaderboardView = document.querySelector<HTMLDivElement>('#leaderboardView')!
  const profileView = document.querySelector<HTMLDivElement>('#playerProfileView')!
...
function renderChart(player: PlayerStats) {
  ...
  new Chart(ctx, {
    ...
    options: {
      ...
      scales: {
        y: { grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#9ea4b0' } },
