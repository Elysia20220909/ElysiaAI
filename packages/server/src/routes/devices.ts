...
const demoDevices: DeviceStatus[] = [
  {
    ...
    networkLatencyMs: 8,
    health: 'nominal',
    notes: ['Primary workstation online', 'GPU temperature within expected range'],
    lastSeenAt: now(),
  },
  {
    id: 'mac-studio-core',
...
function buildFleetReport(devices: DeviceStatus[]) {
  ...
  return {
    ...
    nextActions: [
      ...
      ...warning.map((device) => `Review ${device.name} during next maintenance window`),
