...
class SovereignCrypto {
	...
	constructor() {
		const secret = process.env.ENCRYPTION_SECRET || "elysia-default-shadow-key-777";
	 ...
	 */
	public encrypt(text: string, aad: string = "ELYSIOS"): string {
	 ...
	 */
	public decrypt(encryptedData: string, aad: string = "ELYSIOS"): string {
		try {
			...
			logger.error("🛑 Cryptographic Integrity Violation! (Decryption Failed)", error as Error);
			throw new Error("INTEGRITY_FAILURE: Data may have been tampered with or key mismatch.");
		}
	}
}
