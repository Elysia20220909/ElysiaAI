# Elysia AI ビギナー教科書（最短で動かす編・クリーン版）

「とりあえず動かしながら全体像を掴みたい」人向けのコンパクトガイドです。詳しく学ぶときは `docs/REPO_TEXTBOOK.md`（総合版）と `rust/TEXTBOOK.md`（ネットワーク教材）を参照してください。
...
4. `.env` の最低限を確認/編集
   ...
   - Redis/Milvus はなくても開発を開始可能（必要になったら設定）
