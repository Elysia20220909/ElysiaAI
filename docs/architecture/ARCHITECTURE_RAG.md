...
- `GET /api/suit/os/status`
  - 上位 Linux edge、RTOS MCU、安全リレー、hardware boundary、nanite simulation を束ねた distributed suit OS snapshot を返す。
- `POST /api/suit/os/plan`
  - Mark85 風の自然な request を、policy gate、edge runtime、scheduler、blocked capability つきの plan-only 実行計画に変換する。
