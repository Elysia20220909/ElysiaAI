...
| 対象 | 確認している範囲 | 制約 |
| --- | --- | --- |
| 独自 OS の M1 / M2a / M2b | 指定環境の QEMU で起動情報検査、物理ページ管理、独自ページテーブル、書込み禁止・実行禁止と、Ring 3 の違反したプロセスの停止・正常プロセスの継続を検証 | 固定の 2 プロセス。協調切替のみで CPU 独占は防げない。動的な資源回収・任意のアプリのロードは未実装 |
...
検証条件は [M1](docs/native-os/BOOT_VALIDATION.md) と [M2a](docs/native-os/MEMORY_VALIDATION.md)、[M2b](docs/native-os/USERSPACE_VALIDATION.md)、
