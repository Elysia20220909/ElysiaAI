...
| 対象 | 確認している範囲 | 制約 |
| --- | --- | --- |
| 独自 OS の M1 / M2a | 指定環境の QEMU で起動情報検査、物理ページ管理、独自ページテーブル、書込み禁止・実行禁止の故障を検証 | 全ページがカーネル用。ユーザー空間とプロセス隔離は未実装。故障を診断すると停止する |
| 既存 AI アプリ | ホスト OS 上で動くアプリとして、認証、データ、ツール、通信の境界を設計・検証する | 独自カーネルの保護を利用しない。実際の権限はホストの実行ユーザーや設定に依存する |

独自 OS の実機、Secure Boot、複数プロセスの隔離は保証しません。
未知のプログラムや機密データを安全に扱える OS として運用しないでください。
検証条件は [M1](docs/native-os/BOOT_VALIDATION.md) と [M2a](docs/native-os/MEMORY_VALIDATION.md)、
