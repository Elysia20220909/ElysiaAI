...
function add(severity: Severity, message: string, path?: string, detail?: string) {
...
function isSelf(path: string) {
  return normalized(resolve(path)).endsWith("/scripts/security/scan_glassworm.ts");
}
    ...
    for (const section of sections) {
      ...
      for (const name of Object.keys(deps)) {
        if (knownPackageNames.has(name.toLowerCase())) {
          add("fail", `Known GlassWorm-linked npm package in ${section}`, path, name);
        }
      }
    }
  } catch {
...
function scanExtensionManifest(path: string, text: string) {
  ...
  if (!lowerPath.includes("/.vscode/") && !lowerPath.includes(`${sep}.vscode${sep}`)) return;

  try {
    const manifest = JSON.parse(text);
    const values = [
      ...(Array.isArray(manifest.recommendations) ? manifest.recommendations : []),
    ...
    for (const value of values) {
      if (knownExtensionIds.has(value)) {
        add("fail", "Known GlassWorm-linked VS Code extension reference", path, value);
      }
    }
  } catch {
...
function scanRepoFile(path: string) {
  ...
  for (const indicator of knownText) {
    if (lower.includes(indicator.toLowerCase())) {
      add("fail", "Known GlassWorm indicator string in repository file", path, indicator);
    }
  }
  ...
  if (lower.includes("extensionpack") || lower.includes("extensiondependencies")) {
...
function scanInstalledExtensionDir(path: string) {
  ...
  try {
    const manifest = JSON.parse(text);
    const id = `${String(manifest.publisher ?? "")}.${String(manifest.name ?? "")}`.toLowerCase();
    if (knownExtensionIds.has(id)) {
      add("fail", "Known GlassWorm-linked extension is installed", manifestPath, id);
    }
    ...
    add("warn", "Could not parse installed VS Code extension manifest", manifestPath);
  }
}
...
if (!repoOnly) {
  for (const root of extensionRoots()) {
    ...
    for (const entry of readdirSync(root, { withFileTypes: true })) {
      if (entry.isDirectory()) scanInstalledExtensionDir(join(root, entry.name));
    }
  }
}
...
console.log(`Installed extensions scanned: ${repoOnly ? "skipped" : scannedInstalledExtensions}`);
console.log(`Files with variation selectors: ${variationSelectorFiles}`);
console.log(`Extension relationship manifests seen: ${extensionRelationshipFiles}`);

if (findings.length > 0 && (verbose || failures.length > 0 || warnings.length > 0)) {
  console.log("");
  for (const finding of findings) {
    const where = finding.path ? ` ${finding.path}` : "";
    const detail = finding.detail ? ` (${finding.detail})` : "";
    console.log(`[${finding.severity.toUpperCase()}] ${finding.message}${where}${detail}`);
  }
}

if (failures.length > 0) {
  console.error("");
  console.error(`GlassWorm scan failed: ${failures.length} blocking finding(s).`);
  process.exit(1);
}
