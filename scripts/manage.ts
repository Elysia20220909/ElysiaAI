...
async function runCommand(command: string) {
  switch (command) {
    ...
    case "setup-python":
      ...
      const venvDir = process.platform === "win32" ? ".venv/Scripts" : ".venv/bin";
      const pythonExe = process.platform === "win32" ? "python" : "python3";
      
      if (!await $`which ${pythonExe}`.quiet().nothrow()) {
        console.error(`❌ ${pythonExe} not found. Please install Python 3.11+.`);
        process.exit(1);
      }
    ...
    case "check":
      ...
      const libFiles = await $`grep -r "process.env" packages/server/src/lib --exclude="constants.ts" --exclude="env-validator.ts"`.quiet().nothrow();
      ...
      const legacyScripts = await $`ls scripts/*.ps1 scripts/*.sh`.quiet().nothrow();
      const scriptCount = legacyScripts.stdout.toString().split("\n").filter(Boolean).length;
      if (scriptCount > 10) {
        console.warn(`⚠️ High script bloat: ${scriptCount} legacy scripts remaining in scripts/`);
      }
      break;
    default:
