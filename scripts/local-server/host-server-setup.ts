...
function renderEnvValue(value: string): string {
	if (/[\s#$]/.test(value)) {
		if (value.includes("'"))
			throw new Error("Cannot render .env value containing a single quote");
		return `'${value}'`;
	}
	return value;
}
...
function ensureEnv(): string[] {
	...
	lines = setEnvValue(
		lines,
		"DOMAIN_GRAFANA",
		`grafana.${options.domainSuffix}`,
	);
	lines = setEnvValue(
		lines,
		"DOMAIN_PROMETHEUS",
		`prometheus.${options.domainSuffix}`,
	);
...
function main(): void {
	...
	if (!existsSync(join(options.sourceDir, "compose.yaml"))) {
		throw new Error(
			`compose.yaml not found in source dir: ${options.sourceDir}`,
		);
	}
	if (!existsSync(join(options.sourceDir, ".env.example"))) {
		throw new Error(
			`.env.example not found in source dir: ${options.sourceDir}`,
		);
	}
	...
	console.log(
		`Mode:   ${options.start ? "start" : options.validateOnly ? "validate-only" : "prepare-no-start"}`,
	);
