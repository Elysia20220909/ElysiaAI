...
M3b では、許可されたテスト用資料を RAM から読み取る小さなサービスを追加しました。
カーネルが実際の IPC 送信者の権限を検査し、Ring 3 のサービスが応答します。
...
33 ケースの起動・故障・資源回収・IPC・資料サービス試験に合格しています。条件と実測結果は
...
[M3b RAM 資料サービス](docs/native-os/DOCUMENT_SERVICE_VALIDATION.md) を参照してください。
新規マシンでの環境構築全体、実機、日常利用できる OS としての安定性は未検証です。

この文書は M3b を含む開発ブランチの内容を説明します。
