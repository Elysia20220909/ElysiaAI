...
- `good first issue` ラベルから、小さく始められる課題を探せます。
- Issue本文の"はじめかた"に、ブランチ作成、テスト、Draft PRの導線を置いています。
- 分からない点はIssueコメントで聞いてください。メンテナーは安全性、再現手順、受け入れ基準を優先して確認します。
- Secrets、APIキー、Webhook URL、private RAG contentは貼らないでください。
