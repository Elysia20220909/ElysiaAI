...
class ElysiaCyreneEngine:
    ...
    def __init__(self):
        ...
        # エリシアモード（真我）
        try:
            with open(os.path.join(prompt_dir, "elysia.prompt.txt"), encoding="utf-8") as f:
        ...
        # キュレネモード（知識・洞察）
        try:
            with open(os.path.join(prompt_dir, "cyrene_concept.prompt.txt"), encoding="utf-8") as f:
    ...
    def generate_prompt(self, user_input, recent_memories=None, persona_mode="elysia"):
        ...
        if persona_mode == "cyrene" or any(
            k in user_input.lower() for k in ["/cyrene", "分析して", "論理的", "データ"]
        ):
