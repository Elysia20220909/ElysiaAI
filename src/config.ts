...
export function getEnv(key: string, defaultValue?: string): string {
  ...
  if (isProd && ["SESSION_SECRET", "DB_URL"].includes(key)) {
    throw new Error(`CRITICAL: Missing required production environment variable: ${key}`);
  }
...
export const config = {
  ...
  encryptionSecret: getEnv("ENCRYPTION_SECRET", "elysia-default-shadow-key-777"),
  ...
  emailNotificationsEnabled: getEnv("EMAIL_NOTIFICATIONS_ENABLED", "false") === "true",
  ...
  healthMonitoringEnabled: getEnv("HEALTH_MONITORING_ENABLED", "true") === "true",
