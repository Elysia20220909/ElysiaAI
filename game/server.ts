...
function placeOthello(board: Board, x: number, y: number, player: Player): { board: Board, flipped: number } {
  ...
  for (const [dx,dy] of dirs) {
    let nx = x+dx, ny = y+dy, line: [number,number][] = [];
    while (nx>=0 && nx<8 && ny>=0 && ny<8 && newBoard[ny][nx] === (player===1?2:1)) {
      line.push([nx,ny]); nx+=dx; ny+=dy;
    }
    if (line.length && nx>=0 && nx<8 && ny>=0 && ny<8 && newBoard[ny][nx] === player) {
      for (const [fx,fy] of line) { newBoard[fy][fx] = player; flipped++; }
    }
  }
...
function alphabeta(board: Board, player: Player, depth: number, maximizing: boolean, alpha: number, beta: number, origPlayer: Player): number {
  ...
  if (maximizing) {
    ...
    for (let y=0; y<8; ++y) for (let x=0; x<8; ++x) {
      if (placeOthello(board,x,y,player).flipped) {
        ...
        const evalScore = alphabeta(placeOthello(board,x,y,player).board, nextPlayer, depth-1, false, alpha, beta, origPlayer);
    ...
    for (let y=0; y<8; ++y) for (let x=0; x<8; ++x) {
      if (placeOthello(board,x,y,player).flipped) {
        const nextPlayer = player === 1 ? 2 : 1;
        const evalScore = alphabeta(placeOthello(board,x,y,player).board, nextPlayer, depth-1, true, alpha, beta, origPlayer);
...
function getStrongAIMove(board: Board, player: Player): {x:number,y:number}|null {
  ...
  for (let y=0; y<8; ++y) for (let x=0; x<8; ++x) {
    if (placeOthello(board,x,y,player).flipped) {
      const nextPlayer = player === 1 ? 2 : 1;
      const score = alphabeta(placeOthello(board,x,y,player).board, nextPlayer, 1, false, -Infinity, Infinity, player);
...
function getGodAIMove(board: Board, player: Player): {x:number,y:number}|null {
  ...
  for (let y=0; y<8; ++y) for (let x=0; x<8; ++x) {
    if (placeOthello(board,x,y,player).flipped) {
      const nextPlayer = player === 1 ? 2 : 1;
      const score = alphabeta(placeOthello(board,x,y,player).board, nextPlayer, 3, false, -Infinity, Infinity, player);
      if (score > bestScore) { bestScore = score; bestMove = {x,y}; }
    }
  }
  return bestMove;
...
const app = new Elysia()
  ...
  .post('/game/start', ({ body }) => {
    state = {
      ...
      aiLevel: (body && (body.aiLevel === 'strong' || body.aiLevel === 'god')) ? body.aiLevel : 'random',
      userIds: body && body.userIds ? body.userIds : ['user1','user2']
    };
    clients.clear();
    return state;
  }, {
    body: t.Optional(t.Object({ aiEnabled: t.Boolean(), aiLevel: t.String(), userIds: t.Array(t.String()) }))
  })
  .post('/game/action', ({ body }) => {
    ...
    state.history.push(`Player ${player === 1 ? '黒' : '白'}: (${x},${y}) 反転${result.flipped}個${bonus ? ' 角ボーナス+2' : ''}`);
    state.turn = (player === 1 ? 2 : 1) as Player;
    state.passCount = 0;
    // 勝敗判定
    if (isGameOver(state.board)) {
      const [b, w] = countStones(state.board);
      let winnerId = '';
      if (b > w) { state.winner = '黒'; winnerId = state.userIds ? state.userIds[0] : 'user1'; }
      else if (w > b) { state.winner = '白'; winnerId = state.userIds ? state.userIds[1] : 'user2'; }
      else state.winner = '引き分け';
      ...
      if (state.userIds) {
        ...
        if (b > w) { ranking[u1].win++; ranking[u2].lose++; }
        else if (w > b) { ranking[u2].win++; ranking[u1].lose++; }
        else { ranking[u1].draw++; ranking[u2].draw++; }
      }
    }
    clients.forEach(ws => ws.send(JSON.stringify(state)));
    // AI対戦
    if (state.aiEnabled && !state.winner && state.turn === 2) {
      setTimeout(()=>{
        ...
        if (aiMove) {
          ...
          if (!result.flipped) {
            state.passCount++;
            state.history.push(`不正な手/パス: (${aiAction.x},${aiAction.y}) by ${aiAction.player}`);
            if (state.passCount >= 2) {
              const [b, w] = countStones(state.board);
              if (b > w) state.winner = '黒';
              else if (w > b) state.winner = '白';
          ...
          state.board = result.board;
          let bonus = (aiAction.x === 0 || aiAction.x === 7) && (aiAction.y === 0 || aiAction.y === 7) ? 2 : 0;
          state.history.push(`Player ${aiAction.player === 1 ? '黒' : '白'}: (${aiAction.x},${aiAction.y}) 反転${result.flipped}個${bonus ? ' 角ボーナス+2' : ''}`);
          state.turn = (aiAction.player === 1 ? 2 : 1) as Player;
          state.passCount = 0;
          if (isGameOver(state.board)) {
            const [b, w] = countStones(state.board);
  ...
  .ws('/game/ws', {
    ...
    message(ws: any, msg: string) {
      try {
        ...
        if (typeof action.x === 'number' && typeof action.y === 'number' && typeof action.player === 'number') {
          ...
          state.board = result.board;
          let bonus = (action.x === 0 || action.x === 7) && (action.y === 0 || action.y === 7) ? 2 : 0;
          state.history.push(`Player ${action.player === 1 ? '黒' : '白'}: (${action.x},${action.y}) 反転${result.flipped}個${bonus ? ' 角ボーナス+2' : ''}`);
          state.turn = (action.player === 1 ? 2 : 1) as Player;
          state.passCount = 0;
          if (isGameOver(state.board)) {
            const [b, w] = countStones(state.board);
