...
describeLive("Elysia Network Game API", () => {
	test("ゲーム初期化APIが正常に動作する", async () => {
		const res = await axios.post(`${BASE_URL}/game/start`, {
			...
			agents: [
				{ id: "P1", position: "A", userId: "user1", score: 0 },
				{ id: "P2", position: "C", userId: "user2", score: 0 },
			],
		});
	...
	test("スコア10点で勝者が決定する", async () => {
		await axios.post(`${BASE_URL}/game/start`, {
			...
			agents: [
				{ id: "P1", position: "A", userId: "user1", score: 9 },
				{ id: "P2", position: "C", userId: "user2", score: 0 },
			],
		});
		const res = await axios.post(`${BASE_URL}/game/action`, {
		...
		expect(
			res.data.history.some((h: string) => h.includes("Winner: user1")),
		).toBe(true);
	});
});
