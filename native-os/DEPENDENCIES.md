...
2026-09-13、依存なしのローカル package `elysia-user-probe` を追加した。外部 crate とツールの版は変更しない。
追加の build script は linker script の変更追跡と、指定されたユーザー ELF の存在確認・埋め込みパス設定だけを行う。
インストール、Git hooks、外部通信、認証情報の読み込みは行わない。runner が先に生成した ELF を
カーネルへ渡し、ゲストのパーサーが検査する。生成 ELF のみでは信頼せず、実行時のページ保護も維持する。
既存 CI は変更しない。新しいバイナリ配布や新規マシン全体の環境構築実証は含まない。

## M3e の内部サービス

依存なしの `elysia-document-service` を追加。既存復旧試験のサービス命令を専用ソースへ移した。
